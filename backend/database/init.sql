...
CREATE TABLE IF NOT EXISTS presences (
  ...
  INDEX idx_matiere_id (matiere_id),
  INDEX idx_prof_date (professeur_id, date),
...
CREATE TABLE IF NOT EXISTS demandes_liaison (
  ...
  statut ENUM('en_attente','approuvé','refusé') NOT NULL DEFAULT 'en_attente',
  created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  INDEX idx_utilisateur_id (utilisateur_id),
  INDEX idx_eleve_id (eleve_id),
...
CREATE TABLE IF NOT EXISTS parent_enfants (
  id INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
  parent_id INT UNSIGNED NOT NULL,
  eleve_id INT UNSIGNED NOT NULL,
  created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  UNIQUE KEY uk_parent_eleve (parent_id, eleve_id),
  INDEX idx_parent_id (parent_id),
  INDEX idx_eleve_id (eleve_id),
  CONSTRAINT fk_pe_parent FOREIGN KEY (parent_id) REFERENCES parents(id) ON DELETE CASCADE,
  CONSTRAINT fk_pe_eleve FOREIGN KEY (eleve_id) REFERENCES eleves(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
