...
async function validerDemande(demandeId) {
  ...
  await DemandeLiaison.update({ statut: 'valide', utilisateur_id: utilisateur.id }, { where: { id: demande.id } });
...
async function refuserDemande(demandeId, messageAdmin) {
  const demande = await DemandeLiaison.findByPk(demandeId);
  if (!demande || demande.statut !== 'en_attente') return null;
  await DemandeLiaison.update({ statut: 'refuse', message_admin: messageAdmin || null }, { where: { id: demande.id } });
  return demande;
}
