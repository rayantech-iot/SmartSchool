...
export default function EleveMessages() {
  ...
  async function load() { try { setData(await get('/api/eleve/messages')); } catch {} }
  useEffect(() => { load(); }, []);
  ...
  return (
    <div>
      ...
      {msg.text && <div className={`alert ${msg.type === 'success' ? 'alert-success' : 'alert-error'}`}>{msg.text}<button className="alert-close" onClick={() => setMsg({ type: '', text: '' })}>&times;</button></div>}
        ...
        <div className="modal-overlay" onClick={() => setSelected(null)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div style={{ fontSize: 12, color: '#6b7280', marginBottom: 16 }}>
              ...
              {selected.date_lecture && ` | Lu le ${new Date(selected.date_lecture).toLocaleDateString('fr-FR')}`}
            </div>
