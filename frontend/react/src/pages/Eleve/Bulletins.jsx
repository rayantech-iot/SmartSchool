...
export default function EleveBulletins() {
  const [bulletins, setBulletins] = useState(null);
  useEffect(() => { get('/api/eleve/bulletins').then(d => setBulletins(d.bulletins)); }, []);
  ...
  return (
    <div>
      <div className="page-header-bar">
        <h1 className="page-title">Mes bulletins</h1>
      </div>
      {bulletins.length === 0 ? (
        <div className="empty-state">Aucun bulletin</div>
      ) : (
