// ============================================================
// services/classService.js — Création automatique des classes
// ============================================================
...
/**
 * Prochaine section disponible (A, B, C…) pour un niveau donné
 */
...
/**
 * Salle automatique : ex. LYC-TER-A
 */
...
/**
 * Crée une classe avec section et salle automatiques
 */
async function creerClasse({ cycle, niveau, annee_scolaire }) {
  if (!CYCLES[cycle]) {
    throw new Error('Cycle scolaire invalide.');
  }
  if (!CYCLES[cycle].niveaux.includes(niveau)) {
    throw new Error(`Niveau "${niveau}" invalide pour le cycle ${cycle}.`);
  }

  const annee = annee_scolaire || getAnneeScolaireCourante();
  if (!estAnneeScolaireCourante(annee)) {
    throw new Error(`Seule l'année scolaire courante (${getAnneeScolaireCourante()}) est autorisée.`);
  }
  ...
  return {
    classe,
    libelle: getLibelleComplet(niveau, section)
  };
}

/**
 * Aperçu avant création (sans enregistrer)
 */
async function apercuCreation(cycle, niveau, anneeScolaire) {
  ...
  return {
    libelle: getLibelleComplet(niveau, section),
    section,
    capacite: getCapacitePourCycle(cycle),
    salle: genererSalle(cycle, niveau, section),
    annee_scolaire: annee
  };
}
